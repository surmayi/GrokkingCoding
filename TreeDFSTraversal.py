...
def sum_of_paths_asIntegers(root):
    return sum_of_paths_asIntegers_helper(root, 0)


def sum_of_paths_asIntegers_helper(node, curSum):
    if not node:
        return 0
    ...
    if not node.left and not node.right:
        return curSum
    return sum_of_paths_asIntegers_helper(node.left, curSum) + sum_of_paths_asIntegers_helper(node.right, curSum)
...
def count_paths(root, S):
    return count_paths_helper(root, S, 0)


def count_paths_helper(node, S, curSum):
    if not node:
        return 0
    curSum += node.val
    if not node.left and not node.right and curSum == S:
        return 1
    return count_paths_helper(node.left, S, curSum) + count_paths_helper(node.right, S, curSum)
...
def main():
    ...
    print("4. Max Sum of all paths: " + str(max_sum_path(root)))
